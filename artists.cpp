...
void transformaArtistsBin() // Função que transforma o arquivo artists.csv em binário
{
    ofstream arquivoArtistsBin;
    arquivoArtistsBin.open("artists.csv", ios::binary);
    artists art;
    if (arquivoArtistsBin.is_open())
    {
        for (int i = 0; i < sizeof(art) ; i++)
        {
            arquivoArtistsBin.write((char *) &art[i], sizeof(art))
